...
// 简化的管理员身份验证
async function verifyAdminAuth(token: string, supabaseUrl: string, serviceRoleKey: string) {
...
// 列表查询
async function handleList(table: string, filters: any, supabaseUrl: string, serviceRoleKey: string) {
...
// 创建记录
async function handleCreate(table: string, data: any, supabaseUrl: string, serviceRoleKey: string) {
...
// 更新记录
async function handleUpdate(table: string, id: number, data: any, supabaseUrl: string, serviceRoleKey: string) {
...
// 删除记录
async function handleDelete(table: string, id: number, supabaseUrl: string, serviceRoleKey: string) {
    const response = await fetch(`${supabaseUrl}/rest/v1/${table}?id=eq.${id}`, {
        method: 'DELETE',
        headers: {
            'Authorization': `Bearer ${serviceRoleKey}`,
...
// 批量更新
async function handleBulkUpdate(table: string, items: any[], supabaseUrl: string, serviceRoleKey: string) {
...
// 获取Claude API密钥列表
async function getClaudeApiKeys(supabaseUrl: string, serviceRoleKey: string) {
    try {
...
// 添加Claude API密钥
async function addClaudeApiKey(data: any, supabaseUrl: string, serviceRoleKey: string) {
    try {
...
// 测试Claude API密钥
async function testClaudeApiKey(keyId: number, supabaseUrl: string, serviceRoleKey: string) {
    try {
        // 获取API密钥
...
// 启用Claude API密钥
async function activateClaudeApiKey(keyId: number, supabaseUrl: string, serviceRoleKey: string) {
    try {
        // 先将所有其他密钥设为非启用状态
...
// 删除Claude API密钥
async function deleteClaudeApiKey(keyId: number, supabaseUrl: string, serviceRoleKey: string) {
    try {
        const response = await fetch(`${supabaseUrl}/rest/v1/api_keys?id=eq.${keyId}`, {
            method: 'DELETE',
            headers: {
