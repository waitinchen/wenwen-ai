...
# 測試 3: 第三句（確認會話延續）
Write-Host "🔍 測試 3: 發送第三句訊息（確認會話延續）" -ForegroundColor Green
Write-Host "----------------------------------------" -ForegroundColor Cyan
...
Write-Host "📤 請求內容：" -ForegroundColor Yellow
Write-Host "  - session_id: $SESSION_ID (沿用會話)" -ForegroundColor White
Write-Host "  - message: 謝謝你的推薦！" -ForegroundColor White
Write-Host "  - user_meta: $DISPLAY_NAME, $AVATAR_URL" -ForegroundColor White
Write-Host ""

try {
    $response3 = Invoke-RestMethod -Uri $API_URL -Method POST -Body $body3 -ContentType "application/json" -Headers @{
        "Idempotency-Key" = "33333333-3333-4333-8333-333333333333"
    }
    
    Write-Host "📥 回應內容：" -ForegroundColor Yellow
    $response3 | ConvertTo-Json -Depth 3 | Write-Host -ForegroundColor White
    Write-Host ""
} catch {
    Write-Host "❌ 請求失敗: $($_.Exception.Message)" -ForegroundColor Red
}
...
Write-Host "✅ 第三句：沿用會話 - $SESSION_ID" -ForegroundColor White
Write-Host ""

Write-Host "🎯 驗收檢查點：" -ForegroundColor Yellow
Write-Host "1. 後台「對話列表」應出現：$DISPLAY_NAME + 頭像 + 最後一句預覽" -ForegroundColor White
Write-Host "2. 「對話詳情」這個會話應有 3 條用戶訊息 + 3 條 AI 回覆" -ForegroundColor White
